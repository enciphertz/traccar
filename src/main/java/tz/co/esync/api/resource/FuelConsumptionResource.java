...
public class FuelConsumptionResource extends BaseResource {

    @GET
    public Stream<FuelConsumption> get(
        ...
        if (from != null && to != null) {
            conditions.add(new Condition.Between("startTime", from, to));
        }
        
        Request request = new Request(
            new Columns.All(),
            Condition.merge(conditions),
            new Order("startTime", false, 0)
        );
        
        return storage.getObjectsStream(FuelConsumption.class, request);
    }

    @Path("{id}")
    @GET
    public Response getSingle(@PathParam("id") long id) throws StorageException {
        FuelConsumption fuel = storage.getObject(FuelConsumption.class, new Request(
        ...
        permissionsService.checkPermission(Device.class, getUserId(), fuel.getDeviceId());
        return Response.ok(fuel).build();
    }

    @Path("device/{deviceId}")
    @GET
    public Stream<FuelConsumption> getByDevice(
